...
builder.Services.AddCors(o=>o.AddPolicy("DateMapWeb",p=>p.WithOrigins("http://localhost:5051","http://localhost:5086","https://localhost:5052","http://localhost:5155","http://localhost:5243").AllowAnyHeader().AllowAnyMethod()));
...
var app=builder.Build(); app.UseExceptionHandler(); if(app.Environment.IsDevelopment()){app.UseCors("DateMapWeb");app.UseSwagger();app.UseSwaggerUI();} if(!app.Environment.IsEnvironment("Testing"))app.UseHttpsRedirection();app.UseAuthentication();app.UseAuthorization();app.MapControllers();app.Run();
